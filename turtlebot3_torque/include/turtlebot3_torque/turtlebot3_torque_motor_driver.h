...
#define WAFFLE_DXL_LIMIT_MAX_CURRENT             1193    // MAX CURRENT is 1193(Unit is 2.69 mA)
...
#define CURRENT_CONSTANT_VALUE          0.00269 
...
class Turtlebot3MotorDriver
{
 public:
  ...
  bool writeCurrent(int32_t left_value, int32_t right_value);
  bool controlMotor(const float wheel_radius, const float wheel_separation, float* value);
  bool controlMotor(const float& left_torque, const float& right_torque, float& return_left_current);
