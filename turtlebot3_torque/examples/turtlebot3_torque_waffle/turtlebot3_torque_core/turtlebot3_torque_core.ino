...
#include "turtlebot3_torque_core_config.h"
...
void setup()
{
  ...
  nh.subscribe(cmd_torque_left_sub);
  nh.subscribe(cmd_torque_right_sub);
  ...
  nh.advertise(cmd_torque_left_pub);
  nh.advertise(cmd_torque_right_pub);
  nh.advertise(return_left_value_pub);
  ...
  goal_torque[LEFT] = 0.0;
  goal_torque[RIGHT] = 0.0;
}
...
void loop()
{
  ...
  if ((t-tTime[0]) >= (1000 / CONTROL_MOTOR_TORQUE_FREQUENCY))
  {
    ...
    if ((t-tTime[6]) > CONTROL_MOTOR_TIMEOUT) 
    {
      // no buttons pressed on RC, forcing the robot to stop
      motor_driver.controlMotor(zero_torque[LEFT], zero_torque[RIGHT], return_left_value);
    } 
    else {
      motor_driver.controlMotor(goal_torque[LEFT], goal_torque[RIGHT], return_left_value);
      publishReturnLeftValue();
    }
    */
  if((t - tTime[6]) < CONTROL_MOTOR_TIMEOUT){
    setup();
  ...
  if ((t-tTime[1]) >= (1000 / CMD_PUBLISH_FREQUENCY))
  {
...
/*******************************************************************************
* Callback function for cmd_torque msg
*******************************************************************************/
void commandTorqueCallbackLeft(const std_msgs::Float64& cmd_torque_msg)
{
  goal_torque[LEFT] = (float)cmd_torque_msg.data;
}
void commandTorqueCallbackRight(const std_msgs::Float64& cmd_torque_msg)
{
  goal_torque[RIGHT] = (float)cmd_torque_msg.data;
}
...
*******************************************************************************/
void publishCmdTrqLeft(void)
{
  cmd_torque_left_msg.data = goal_torque[LEFT];
  cmd_torque_left_pub.publish(&cmd_torque_left_msg);
}

void publishCmdTrqRight(void)
{
  cmd_torque_right_msg.data = goal_torque[RIGHT];
  cmd_torque_right_pub.publish(&cmd_torque_right_msg);
}

void publishReturnLeftValue(void)
{
  //return_left_value_msg.data = return_left_value;
  return_left_value_msg.data = millis();
  return_left_value_pub.publish(&return_left_value_msg);
}
