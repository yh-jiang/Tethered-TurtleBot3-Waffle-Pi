...
#include "../../include/turtlebot3_torque/turtlebot3_torque_motor_driver.h"
...
bool Turtlebot3MotorDriver::setMode()
{
  uint8_t dxl_error = 0;
  int dxl_comm_result = COMM_TX_FAIL;

  dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, DXL_LEFT_ID, ADDR_OPERATING_MODE, OPERATING_MODE, &dxl_error);
  ...
  dxl_comm_result = packetHandler_->write1ByteTxRx(portHandler_, DXL_RIGHT_ID, ADDR_OPERATING_MODE, OPERATING_MODE, &dxl_error);
  if(dxl_comm_result != COMM_SUCCESS)
  {
    Serial.println(packetHandler_->getTxRxResult(dxl_comm_result));
    return false;
...
bool Turtlebot3MotorDriver::writeCurrent(int32_t left_value, int32_t right_value)
{
  bool dxl_addparam_result;
  int8_t dxl_comm_result;
...
bool Turtlebot3MotorDriver::controlMotor(const float& left_torque, const float& right_torque, float& return_left_current)
{
  bool dxl_comm_result = false;
  ...
  float right_value;
  /*
  if(left_torque < 0.14 && left_torque > -0.14)
    left_value = 0.0;
  else{
    if(left_torque >= 0.14)
      left_value  = 0.2567463 * left_torque * left_torque + 0.4465496 * left_torque + 0.2715388;
    else
      left_value  = -(0.2567463 * left_torque * left_torque - 0.4465496 * left_torque + 0.2715388);
    left_value  = left_value/CURRENT_CONSTANT_VALUE;
  }

  if(right_torque < 0.14 && right_torque > -0.14)
    right_value = 0.0;
  else{
    if(right_torque >= 0.14)
      right_value = 0.2567463 * right_torque * right_torque + 0.4465496 * right_torque + 0.2715388;
    else
      right_value = -(0.2567463 * right_torque * right_torque - 0.4465496 * right_torque + 0.2715388);
    right_value = right_value/CURRENT_CONSTANT_VALUE;
  }
  */
  ...
  left_value  = constrain(left_value, -dynamixel_limit_max_current_, dynamixel_limit_max_current_);
  right_value = constrain(right_value, -dynamixel_limit_max_current_, dynamixel_limit_max_current_);

  return_left_current = left_value;

  dxl_comm_result = writeCurrent((int32_t)left_value, (int32_t)right_value);
  if (dxl_comm_result == false)
    return false;
